...
vec4 fog() {
  ...
  vec4 fogPixel = mix(vec4(color, 1.), vec4(1.5, 1.5, 1.5, 1.5), clamp(length(r.x), .4, 1.));

  return (f * f * f + .6 * f * f + .5 * f) * fogPixel;
}
...
void main(void) {
  ...
  vec4 fogPixel = contrastMatrix(3.) * fog();
